...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  train_data_node = tf.placeholder(
      data_type(),
      shape=(1, ROW, COL, NUM_CHANNELS))
  train_labels_node = tf.placeholder(tf.int64, shape=(1,))
  ...
  with tf.Session() as sess:
    ...
    # Save my model to predict
    saver = tf.train.Saver()
    saver.restore(sess,'./logs/mnist.tfmodel-2100')
    for step in xrange(int(num_epochs * train_size) // BATCH_SIZE):
      # Compute the offset of the current minibatch in the data.
      # Note that we could use better randomization across epochs.
      
      elapsed_time = time.time() - start_time
      folder = raw_input("continue test,Please input 0-9: ")
      start_time = time.time()
      mnist = input_data(train_dir='/home/a/workspace/ssd/DataSets/'+folder+'/',\
                    test_dir='/home/a/workspace/ssd/DataSets/mnist_120_test/',\
                   num_class=10, one_hot=False, validation_size=1)
      ...
      print("prediction is : ",numpy.argmax(predictions, 1))
      print("\n\n")
      sys.stdout.flush()
